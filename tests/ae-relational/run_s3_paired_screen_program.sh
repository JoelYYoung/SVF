...
run_one()
{
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$order" "$config" "$sparsity" "$domain" "$policy" "$call_policy" \
...
test "$(($(wc -l < "$results") - 1))" -eq 2 || {
  gate_status=1
  gate_reason=row-count
}
completed=$(awk -F '\t' 'NR > 1 && $18 == "completed" { count++ } END { print count+0 }' "$results")
if ((completed == 2)); then
  gate_reason=passed
  awk -F '\t' 'NR > 1 && ($17 != 0 || $44 != 0 || $45 != 0) { bad++ }
    END { exit bad != 0 }' "$results" || {
      gate_status=1
      gate_reason=post-or-status
    }
  identities=$(tail -n +2 "$results" | cut -f39 | LC_ALL=C sort -u | wc -l)
