...
def run_variant(executable, extapi, bitcode, cap, directory):
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"observer failed with {completed.returncode}: {directory}"
        )
...
def main():
    ...
    variants = ("original-pr1887", "whole", "chunk")
    runs = collections.defaultdict(list)
    run_records = []
    for repeat in range(2):
        order = variants if repeat == 0 else tuple(reversed(variants))
    ...
    upstream_pr1887 = differences(upstream, runs["original-pr1887"][0])
    result = {
        "schema": 1,
        ...
        "pr1887_whole_strict_exact": not pr1887_whole,
        "pr1887_whole_compatible": not disallowed,
        ...
        "upstream_pr1887_difference_count": len(upstream_pr1887),
        "upstream_pr1887_differences": upstream_pr1887,
    }
    result_file = output / "result.json"
    result_file.write_text(json.dumps(result, indent=2) + "\n")
    if not all(repeat_stable.values()):
    ...
    print(
        ...
        f"pr1887_whole_strict_differences={len(pr1887_whole)}",
        "pr1887_whole_compatible=true",
        f"upstream_pr1887_differences={len(upstream_pr1887)}",
