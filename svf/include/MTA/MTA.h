...
public:
    ...
    /// Artifact (msli-branch) race-alarm metric: distinct racy statements
    /// deduplicated by raceStmtKey (enclosing function + store/load) -- a
    /// source-level count, deliberately coarser than the raw IR-statement count.
